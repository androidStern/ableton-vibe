...
import { abletonApiMap } from './generated/abletonApiMap.js'
...
async function main() {
  ...
  try {
    ...
    /**
     * Experimental Resource: 'ableton://song/{property}' that returns the value of a song property generated by codegen.
     */
    server.resource(
      'song_property',
      new ResourceTemplate('ableton://song/{property}', {
        list: async () => ({
          resources: abletonApiMap.Song.gettableProperties.map(p => ({
            uri: `ableton://song/${p.name}`,
            name: `Song ${p.name}`,
            description: `The ${p.name} property of the current song`
          }))
        }),
        complete: {
          property: async (partial: string) => {
            return abletonApiMap.Song.gettableProperties
              .map(p => p.name)
      ...
      async (uri, { property }) => {
        logger.debug('Resource requested: ableton://song/{property}', { uri: uri.href, property })
        const value = await AbletonConnection.getInstance()
          .getAbleton()
          .song.get(property as any)
        return {
          contents: [{ uri: uri.href, text: String(value) }]
        }
      }
    )
    ...
    server.tool(
      ...
      async ({ trackIndex, fromTime, fromPitch, timeSpan, pitchSpan }) => {
        logger.debug('Tool invoked: delete_midi_notes', {
          trackIndex,
          fromTime,
          fromPitch,
          timeSpan,
        ...
        try {
          ...
          try {
            ...
            logger.debug('Attempting to remove notes in range', {
              fromTime,
              fromPitch,
              timeSpan,
              pitchSpan
          ...
          return {
            content: [
              {
                type: 'text',
                text: `Error deleting MIDI notes: ${
                  error instanceof Error ? error.message : 'Unknown error'
                }`
              }
            ]
